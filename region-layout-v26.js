/* v32 — standard regional injection layout; numbering handled elsewhere. */
(() => {
  ...
  function ensure(){
    ...
    if(outer?.dataset.regionLandmark==='v32')return;
    if(!outer){
      const secs=[...view.querySelectorAll(':scope > .section')].filter(s=>matches(titleOf(s),rs));
      if(secs.length<2)return;
      outer=document.createElement('section');outer.className='section region-landmark-section';const h=document.createElement('h2');h.textContent='Landmark-guided injection techniques';outer.appendChild(h);secs[0].before(outer);
      secs.forEach(s=>{outer.appendChild(card(titleOf(s),[...bodyOf(s).children],s.id));s.remove();});
    }
