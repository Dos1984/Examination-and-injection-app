/* v34 — standard regional injection layout; numbering handled elsewhere. */
(() => {
  ...
  const patterns={
    ...
    ankle:[/ankle joint|tibiotalar/i,/subtalar|sinus tarsi/i,/posterior tibialis|tibialis posterior/i,/plantar fascia/i,/morton/i,/metatarsophalangeal|\bMTP\b/i],
    foot:[/ankle joint|tibiotalar/i,/subtalar|sinus tarsi/i,/posterior tibialis|tibialis posterior/i,/plantar fascia/i,/morton/i,/metatarsophalangeal|\bMTP\b/i]
  };
  ...
  const headingSelector=':scope > h3.sub,:scope > h3,:scope > h4.sub,:scope > h4';

  function card(title,nodes,id){
    const c=document.createElement('section');c.className='hand-inj-sub region-inj-sub collapsed';c.dataset.regionProcedure='v34';if(id)c.id=id;
  ...
  function splitProcedureHeadings(outer,source,sectionId,sectionTitle,rs){
    const procHeads=[...source.querySelectorAll(headingSelector)].filter(h=>matches(h.textContent,rs));
    if(!procHeads.length)return false;
    const headSet=new Set(procHeads);
    const intro=[];let n=source.firstElementChild;
    while(n&&n!==procHeads[0]){const next=n.nextElementSibling;intro.push(n);n=next;}
    procHeads.forEach((h,i)=>{
      const nodes=i===0?[...intro]:[];
      let item=h.nextElementSibling;
      while(item&&!headSet.has(item)){const next=item.nextElementSibling;nodes.push(item);item=next;}
      let title=clean(h.textContent);
      if(route()==='elbow'){
        const base=clean(sectionTitle).replace(/:\s*landmark approaches?$/i,'');
        if(base&&!new RegExp(base.replace(/[.*+?^${}()|[\]\\]/g,'\\$&'),'i').test(title)) title=`${base}: ${title}`;
      }
      outer.appendChild(card(title,nodes,h.id||(i===0?sectionId:'')));
      h.remove();
    });
    return true;
  }

  function addProcedureCards(outer,section,rs){
    const source=bodyOf(section);
    const shouldSplit=['elbow','foot','ankle'].includes(route());
    if(shouldSplit&&splitProcedureHeadings(outer,source,section.id,titleOf(section),rs)){section.remove();return;}
    outer.appendChild(card(titleOf(section),[...source.children],section.id));section.remove();
  }

  function convertFlowHeadings(outer,rs){
    const body=bodyOf(outer);
    const procHeads=[...body.querySelectorAll(headingSelector)].filter(h=>matches(h.textContent,rs));
    const headSet=new Set(procHeads);
    procHeads.forEach(h=>{
      const nodes=[];let n=h.nextElementSibling;
      while(n&&!headSet.has(n)){const next=n.nextElementSibling;nodes.push(n);n=next;}
      h.replaceWith(card(h.textContent,nodes,h.id));
    });
  }
  ...
  function ensure(){
    ...
    if(outer?.dataset.regionLandmark==='v34')return;
    if(!outer){
      ...
      secs.forEach(s=>addProcedureCards(outer,s,rs));
    }
    convertFlowHeadings(outer,rs);
    ...
    outer.dataset.regionLandmark='v34';
  }
